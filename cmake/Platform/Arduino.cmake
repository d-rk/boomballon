...
function(GENERATE_ARDUINO_LIBRARY INPUT_NAME)
    ...
    get_arduino_flags(ARDUINO_COMPILE_FLAGS ARDUINO_LINK_FLAGS ARDUINO_INCLUDE_DIRS ${INPUT_BOARD} ${INPUT_MANUAL})
...
function(GENERATE_ARDUINO_FIRMWARE INPUT_NAME)
    ...
    parse_generator_arguments(${INPUT_NAME} INPUT
                              ...
                              "SERIAL;SRCS;HDRS;LIBS;ARDLIBS;AFLAGS;INCLUDE_DIRS"  # Multi Value Keywords
                              ${ARGN})
    ...
    setup_arduino_target(${INPUT_NAME} ${INPUT_BOARD} "${ALL_SRCS}" "${ALL_LIBS}" "${LIB_DEP_INCLUDES}" "" "${INPUT_INCLUDE_DIRS}" "${INPUT_MANUAL}")
...
function(GENERATE_ARDUINO_EXAMPLE INPUT_NAME)
    parse_generator_arguments(${INPUT_NAME} INPUT
                              ...
                              "SERIAL;AFLAGS;INCLUDE_DIRS"             # Multi Value Keywords
                              ${ARGN})
    ...
    setup_arduino_target(${INPUT_NAME} ${INPUT_BOARD}  "${ALL_SRCS}" "${ALL_LIBS}" "${LIB_DEP_INCLUDES}" "" "${INPUT_INCLUDE_DIRS}" FALSE)
...
function(REGISTER_HARDWARE_PLATFORM PLATFORM_PATH)
    ...
    if(PLATFORM)
		...
		if(EXISTS "${PLATFORM_PATH}/avr" AND IS_DIRECTORY "${PLATFORM_PATH}/avr")
...
#       INCLUDE_DIRS_VAR - Variable holding include directories
...
#
#=============================================================================#
function(get_arduino_flags COMPILE_FLAGS_VAR LINK_FLAGS_VAR INCLUDE_DIRS_VAR BOARD_ID MANUAL)
    ...
    if(BOARD_CORE)
        ...
        if(NOT MANUAL)
            set(INCLUDE_DIRS ${INCLUDE_DIRS} ${${BOARD_CORE}.path} ${ARDUINO_LIBRARIES_PATH})
        endif()
        ...
        if(ARDUINO_SDK_VERSION VERSION_GREATER 1.0 OR ARDUINO_SDK_VERSION VERSION_EQUAL 1.0)
            if(NOT MANUAL)
                ...
                if(PIN_HEADER)
                    set(INCLUDE_DIRS ${INCLUDE_DIRS} ${PIN_HEADER})
                endif()
            endif()
        endif()
...
function(setup_arduino_core VAR_NAME BOARD_ID)
    ...
    if(BOARD_CORE)
        if(NOT TARGET ${CORE_LIB_NAME})
            ...
            get_arduino_flags(ARDUINO_COMPILE_FLAGS ARDUINO_LINK_FLAGS ARDUINO_INCLUDE_DIRS ${BOARD_ID} FALSE)
...
#
#=============================================================================#
function(setup_arduino_target TARGET_NAME BOARD_ID ALL_SRCS ALL_LIBS COMPILE_FLAGS LINK_FLAGS INCLUDE_DIRS MANUAL)
    ...
    get_arduino_flags(ARDUINO_COMPILE_FLAGS ARDUINO_LINK_FLAGS ARDUINO_INCLUDE_DIRS ${BOARD_ID} ${MANUAL})
    set(ARDUINO_INCLUDE_DIRS ${ARDUINO_INCLUDE_DIRS} ${INCLUDE_DIRS})
...
function(SETUP_ARDUINO_EXAMPLE TARGET_NAME LIBRARY_NAME EXAMPLE_NAME OUTPUT_VAR)
    ...
    get_property(LIBRARY_SEARCH_PATH
                 DIRECTORY     # Property Scope
                 PROPERTY LINK_DIRECTORIES)
    foreach(LIB_SEARCH_PATH ${LIBRARY_SEARCH_PATH} ${ARDUINO_AVR_LIBRARIES_PATH} ${ARDUINO_LIBRARIES_PATH} ${CMAKE_CURRENT_SOURCE_DIR} ${CMAKE_CURRENT_SOURCE_DIR}/libraries)
...
if(NOT ARDUINO_FOUND AND ARDUINO_SDK_PATH)
    ...
    find_file(ARDUINO_AVR_LIBRARIES_PATH
        ...
        DOC "Path to directory containing the Arduino AVR libraries.")
