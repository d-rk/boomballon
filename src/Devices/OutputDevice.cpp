...
void OutputDevice::applyPositive(uint8_t intensity, uint16_t durationMs) {


//    if (intensity > 0) {
//        analogWrite(PIN_MOTOR, MOTOR_MIN +  (uint8_t)(factor * intensity));

//        for (uint16_t passed = 0; passed < durationMs; passed += 50) {
//            delay(50);
//            unsigned long millisNow = millis();
//            volume += getVolume(currentValue, millisNow - lastValueChange);
//            lastValueChange = millisNow;
...
void OutputDevice::applyNegative(uint8_t intensity, uint16_t durationMs) {

//    if (intensity > 0) {
//        uint8_t gapMs = (intensity / 6) * 5; // range [0-105] step 5
...
//        uint16_t iterations = (durationMs * gapFactor) / (2*gapMs);
//        float deflateTime = DEFLATE_TIME * gapFactor;
//        float deflateIteration = (2*gapMs) / deflateTime * 100;
